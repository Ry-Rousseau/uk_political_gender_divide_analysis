...
Interpreting the scales: - agreement with the social questions implies high social conservatism - agreement with the economic questions implise high economic leftism
...
Come back to this at a later stage. Enough results for now. Thinking big picture.
...
Need to narrow down an effective methodology for examining segmented trends within the survey model approach.
...
- Specifically, who is becoming more right-wing? Are there any predictors of actual shifts
...
library(survey) design \<- svydesign( ids = \~cluster_var, \# clustering variable (use \~1 if no clustering) strata = \~strata_var, \# stratification variable (optional) weights = \~weight_var, \# survey weights data = your_data )
...
-   you really need to make sure you are following all survey-design inferences, like weighted t tests, standard errors accounting for stratification, clustering and unequal selection probabilities. Generally need to refresh these points

-   how should you make claims and interpretations in survey inference? and really understand what weights mean in the survey context?

-   What's the exact difference between survey glm and just a normal linear model? like those found in survey package. What is the point of the design objects? what is the clustering parameters? 

-   review multilevel regression with poststratification

-   come back to the survey design when you figure out stratification more explicitly to ensure you are doing it correctly

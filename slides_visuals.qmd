...
plot_df_faceted <- plot_df %>%
  mutate(
    panel = if_else(mean_signed < 0, "Economic rightism", "Social Conservatism"),
...
# Filter for the four main parties and create visualization
major_parties_survey <- vote_proportions_survey %>%
  filter(party %in% c("con", "rfm", "lab", "ldm")) %>%
  mutate(
    party_label = case_when(
      party == "con" ~ "Conservative",
      party == "rfm" ~ "Reform",
      party == "lab" ~ "Labour", 
      party == "ldm" ~ "Liberal Democrat"
    ),
...
ggplot(major_parties_survey, aes(x = age, y = vote_share, fill = gender)) +
  ...
  facet_wrap(~party_label, scales = "free_y", nrow = 2) +
  ...
  labs(
    x = NULL, 
    y = "Vote Share (%)",
    title = "2024 Vote Share by Gender and Age Group"
  ) +
  theme_minimal(base_size = 12, base_family = "sans") +
  theme(
...
ggplot(gap_plot_data, aes(x = age, y = gap_value, fill = gap_type)) +
  ...
  scale_fill_manual(
    values = c("social_gap" = "#E76F51", "economic_gap" = "#2A9D8F"),
    guide = "none"
  ) +
  scale_x_discrete(labels = c("18–24", "25–34", "35–44", "45–54", "55–64", "65–74", "75+")) +
...
ggplot(combined_plot_data, aes(x = issue_label)) +
  ...
  scale_fill_manual(
    values = c("Female" = "#E76F51", "Male" = "#2A9D8F"),
    name = "Gender"
  ) +
...
### Slide 4: Breakdown of the 18-30 Age Group - Cluseters/ Segments

-   Demonstrate clear evidence that young men and women have fundamentally different worldviews
-   Support argument that young people are NOT homogenous
...
# Function to generate predictions for demographic segments
predict_ideological_segments <- function(
  models = list(social = model_social, economic = model_economic),
  demographics = list(),
  baseline_profile = "modal",
  output_format = "long"
) {
  
  # Define modal (most common) values for baseline
  modal_values <- list(
    gender = "female",
    age = "35_TO_44", 
    education = "low",
    ...
    household = "owns_with_mortgage_or_shared",
    socgrad = "C1",
    marital = "married_civil",
    religion = "none"
  )
  
  # Set baseline profile
  if (baseline_profile == "modal") {
    baseline <- modal_values
  } else {
    baseline <- c(modal_values, baseline_profile)
  }
  
  # Create all combinations of specified demographics
  if (length(demographics) == 0) {
    stop("Must specify at least one demographic variable to vary")
  }
  
  # Generate grid of demographic combinations
  demo_grid <- expand_grid(!!!demographics)
  
  # Create prediction dataframe by combining baseline with each demo combination
  pred_data <- demo_grid %>%
    rowwise() %>%
    do({
      # Start with baseline
      row_data <- baseline
      # Override with specific demographic values for this row
      for (var in names(demographics)) {
        row_data[[var]] <- .[[var]]
      }
  ...
  social_pred <- predict(models$social, newdata = pred_data, se.fit = TRUE)
  economic_pred <- predict(models$economic, newdata = pred_data, se.fit = TRUE)
  
  # Extract predictions and standard errors properly
  if (is.list(social_pred)) {
    social_fit <- social_pred$fit
    social_se <- social_pred$se.fit
  } else {
    social_fit <- as.numeric(social_pred)
    social_se <- rep(NA, length(social_fit))
  }
  
  if (is.list(economic_pred)) {
    economic_fit <- economic_pred$fit
    economic_se <- economic_pred$se.fit
  } else {
    economic_fit <- as.numeric(economic_pred)
    economic_se <- rep(NA, length(economic_fit))
  }
  
  # Combine predictions with demographic data
  results <- pred_data %>%
    select(all_of(names(demographics))) %>%
    mutate(
      social_conservatism_pred = social_fit,
      social_conservatism_se = social_se,
      social_conservatism_lower = social_conservatism_pred - 1.96 * social_conservatism_se,
      social_conservatism_upper = social_conservatism_pred + 1.96 * social_conservatism_se,
      economic_rightism_pred = economic_fit,
      economic_rightism_se = economic_se,
      economic_rightism_lower = economic_rightism_pred - 1.96 * economic_rightism_se,
      economic_rightism_upper = economic_rightism_pred + 1.96 * economic_rightism_se,
  ...
  if (output_format == "long") {
    results <- results %>%
      pivot_longer(
        cols = c(social_conservatism_pred, economic_rightism_pred),
        names_to = "ideology_measure",
        values_to = "predicted_value",
        names_pattern = "(.*)_pred"
      ) %>%
      mutate(
        se = case_when(
          ideology_measure == "social_conservatism" ~ social_conservatism_se,
          ideology_measure == "economic_rightism" ~ economic_rightism_se
        ),
        ci_lower = case_when(
          ideology_measure == "social_conservatism" ~ social_conservatism_lower,
          ideology_measure == "economic_rightism" ~ economic_rightism_lower
        ),
        ci_upper = case_when(
          ideology_measure == "social_conservatism" ~ social_conservatism_upper,
          ideology_measure == "economic_rightism" ~ economic_rightism_upper
        )
      ) %>%
      select(-ends_with("_se"), -ends_with("_lower"), -ends_with("_upper"))
  }
  
  return(results)
}

# Test the function with gender comparison
gender_predictions <- predict_ideological_segments(
  demographics = list(gender = c("male", "female"))
)

print("Gender comparison predictions:")
print(gender_predictions)

# Test with age groups
age_predictions <- predict_ideological_segments(
  demographics = list(age = c("18_TO_24", "35_TO_44", "65_TO_74"))
)

print("Age group predictions:")
print(age_predictions)

# Test with gender x education interaction
gender_education_predictions <- predict_ideological_segments(
  demographics = list(gender = c("male", "female"), education = c("low", "high"))
)

print("Gender x Education predictions:")
print(gender_education_predictions)

# Function to create horizontal bar charts from predictions dataframe
plot_segment_predictions <- function(
  predictions_df, 
  title = "Predicted Ideological Scores by Segment",
  facet_by_measure = TRUE,
  color_var = NULL,
  format = "wide"
) {
    ...
    plot_data <- predictions_df %>%
      ...
      mutate(
        ideology_label = case_when(
          ideology_measure == "social_conservatism" ~ "Social Conservatism",
          ideology_measure == "economic_rightism" ~ "Economic Rightism"
        )
      )
  } else {
  ...
  # Detect demographic variables (exclude prediction columns)
  demo_vars <- setdiff(names(predictions_df), 
                      c("social_conservatism_pred", "economic_rightism_pred", 
                        "social_conservatism_se", "economic_rightism_se",
                        "social_conservatism_lower", "social_conservatism_upper",
                        "economic_rightism_lower", "economic_rightism_upper",
                        "segment_id", "ideology_measure", "predicted_value",
                        "se", "ci_lower", "ci_upper", "ideology_label"))
  
  # Create segment labels by combining all demographic variables
  if (length(demo_vars) == 1) {
    plot_data <- plot_data %>%
      mutate(segment_label = .data[[demo_vars[1]]])
  } else if (length(demo_vars) == 2) {
    plot_data <- plot_data %>%
      mutate(segment_label = paste(.data[[demo_vars[1]]], .data[[demo_vars[2]]], sep = " × "))
  } else {
    plot_data <- plot_data %>%
      rowwise() %>%
      mutate(segment_label = paste(c_across(all_of(demo_vars)), collapse = " × ")) %>%
      ungroup()
  }
  
  # Determine color variable
  if (is.null(color_var) && length(demo_vars) >= 1) {
    color_var <- demo_vars[1]  # Use first demographic variable for coloring
  }
  
  # Create base plot
  p <- ggplot(plot_data, aes(x = predicted_value, y = reorder(segment_label, predicted_value)))
  
  # Add color aesthetic if specified
  if (!is.null(color_var) && color_var %in% names(plot_data)) {
    p <- p + geom_col(aes(fill = .data[[color_var]]), alpha = 0.8)
    
    # Use consistent colors if gender is involved
    if (color_var == "gender") {
      p <- p + scale_fill_manual(
        values = c("male" = "#2A9D8F", "female" = "#E76F51"),
        name = "Gender"
      )
    }
  } else {
    p <- p + geom_col(fill = "#2A9D8F", alpha = 0.8)
  }
  ...
  if (facet_by_measure && "ideology_label" %in% names(plot_data)) {
    p <- p + facet_wrap(~ ideology_label, scales = "free_x")
  }
...
gender_plot <- plot_segment_predictions(
  gender_predictions, 
  title = "Gender Differences in Political Attitudes",
  format = "long",
  color_var = "gender"
)
...
age_plot <- plot_segment_predictions(
  age_predictions,
  title = "Political Attitudes Across Age Groups", 
  format = "long",
  color_var = "age"
)
...
interaction_plot <- plot_segment_predictions(
  gender_education_predictions,
  title = "Gender × Education Effects on Political Attitudes",
  format = "long", 
  color_var = "gender"
)
...
london_vs_rest <- gender_gap_by_region_age %>%
  ...
  mutate(
    gap_type = case_when(
      ...
      gap_type == "economic_gap" ~ "Economic Rightism Gap"
    )
  )
